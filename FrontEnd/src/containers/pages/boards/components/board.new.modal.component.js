...
class BoardNewModal extends React.Component {
  state = {
    data: {
      name: '',
      votes: 6,
  ...
  UNSAFE_componentWillReceiveProps(nextProps) {
    ...
    if (nextProps.res_data.api === 'templatelist') {
      let template = '';
      if (nextProps.res_data.data.length) {
        template = nextProps.res_data.data[0]._id;
      }
      const board_data = this.state.data;
      board_data.template = template;
      this.setState({
    ...
    return (
      ...
      >
        <DialogTitle className={scss['board-modal-title']}>
          {"Create Board"}
          <IconButton onClick={handleClose} disabled={loading_flag} className={scss['board-modal-close-btn']}>
            <CloseIcon/>
          </IconButton>
        </DialogTitle>
...
export default (connect(mapStateToProps, { getTemplateList }))(BoardNewModal);
