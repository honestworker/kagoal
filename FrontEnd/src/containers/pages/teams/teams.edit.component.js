...
import { getTeamInfo, getTeamTreeList, setTeamInfo } from '../../../actions/teams.action';
...
class TeamsEdit extends React.Component {
  ...
  componentDidMount = () => {
    this.props.getTeamInfo(this.state.id);
  ...
  getTeamsTree = (teams) => {
    const teams_tree = [];
    const team_id = this.state.id;
    function _getTeamsTree(team, team_deep = [], disble_flag = false) {
      if (team) {
        ...
        if (team.children.length) {
          team.children.map((team_child_loop, team_child_indx) => {
            ...
            teams_tree_children.push({
              ...
              children: [],
              disabled: disble_flag
            });
            team_deep.push(team_child_indx);
            _getTeamsTree(team_child_loop, team_deep, disble_flag);
            team_deep.pop();
          });
        }
      }
    ...
    if (teams) {
      if (teams.length) {
        ...
        teams.map((team_loop, team_indx) => {
          let main_disable_flag = false;
          if (team_loop.id === team_id) {
            main_disable_flag = true;
          }
          teams_tree.push({
  ...
  UNSAFE_componentWillReceiveProps(nextProps) {
    ...
    if (nextProps.res_data.api === 'getteam') {
      const res_team_data = nextProps.res_data.data;
      const team_data = this.state.data;
      team_data.id = res_team_data.id;
      team_data.name = res_team_data.name;
      team_data.description = res_team_data.description;
      team_data.board = res_team_data.board;
      team_data.parentId = res_team_data.parentId;
      this.setState({
        data: team_data
      })
    }

    if (nextProps.res_data.api === 'updateteam') {
    ...
    const handleSubmit = (event) => {
      ...
      setTimeout(()=> {
        if (checkNoError()) {
          ...
          this.props.setTeamInfo(submit_data);
        }
      }, 300);
    };
...
export default withRouter(compose(withWidth(), withStyles(themeStyles, { withTheme: true }), connect(mapStateToProps, { getTeamInfo, getTeamTreeList, getBoardList, setTeamInfo }))(TeamsEdit));
